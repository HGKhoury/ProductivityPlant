...
void movePlant(int angle);
void celebrationWiggle(void);
void servoResetToUpright();
void moveDown();
...
void loop(){
  if (buttonPressed) {
    if (!breakRunning) {
        if (isProductive) {
            showText("UR BEING PRODUCTIVE!");
            servoResetToUpright();
        }
        else {
            showText("GET BACK TO WORK!");
            moveDown();
        }
    }
    ...
    if (millis() - deepWorkStartTime >= deepWorkPeriod) {
        ...
        if (elapsedBreakTime <= breakPeriod) {
            // break time running
            Serial.println("wiggle during break");
            serv1.write(90);
            serv2.write(90);
            delay(500);
            ...
            // break time over
            breakRunning = false;
            showText("press button to restart deep work");
            buttonPressed = false;
        }
    }
  }
